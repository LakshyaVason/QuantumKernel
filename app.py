...
# ----------------- Decision Boundary (OPTIMIZED) -----------------
def plot_decision_boundary(clf, X_train, y_train, feature_map, ax):
    # MUCH COARSER GRID - this is the key change!
    h = 0.25  # Changed from 0.05 to 0.25 (5x fewer points per dimension = 25x faster!)
    x_min, x_max = X_train[:,0].min()-0.3, X_train[:,0].max()+0.3  # Smaller boundary too
    ...
    print(f"Grid size: {len(grid)} points")  # Debug info
...
def run_kernel():
    try:
        ...
        depth = min(int(params['depth']), 3)  # Cap depth at 3 to prevent extreme slowness
        ent = params['ent']
